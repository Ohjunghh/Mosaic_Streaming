...
#yolo_model = None  # 글로벌로 YOLO 모델을 선언
...
#             yolo_model = torch.hub.load('ultralytics/yolov5', 'custom', path='./face_recognition_2/627.pt', force_reload=True)
#             yolo_model.to(device)
#             yolo_model.conf = confidence_t
#             yolo_model.classes = None
#             yolo_model.agnostic_nms = False
#             logger.info('YOLO model loaded globally.')
...
class WebcamStream:
    def __init__(self, stream_id):
        ...
        #     '-c:a', 'aac',  # 오디오 코덱?
        #     '-b:a', '128k',  # 오디오 비트레이트
        #     f'rtmp://172.30.1.26/live-out/{self.stream_id}'#'rtmp://192.168.58.194/live-out/{self.stream_id}'
        # ]

    # def load_model(self):
    #     device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    #     logger.info(f'Using device: {device}')  # 디바이스 정보 로그 출력
    #     self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='./face_recognition_2/627.pt', force_reload=True)
    #     self.model.to(device)  # 모델을 GPU로 보내기
    #     self.model.conf = confidence_t
       ...
       # GPU로 모델을 옮길 때
        try:
            with tf.device('/GPU:0'):  # GPU 사용
                self.face_encoder = self.face_encoder  # Keras에서는 별도의 GPU 전송 메서드가 필요 없음
        except RuntimeError as e:
            logger.error(f"Error moving the face encoder model to GPU: {e}")
            
    # def load_encoding_dict(self):
    #     with open('./face_recognition_2/encodings/encodings.pkl', 'rb') as f:
    ...
    def start_ffmpeg(self):
        try:
            # FFmpeg 프로세스 실행
            self.process = subprocess.Popen(self.command, stdin=subprocess.PIPE, stderr=subprocess.PIPE)#stderr=subprocess.PIPE)
