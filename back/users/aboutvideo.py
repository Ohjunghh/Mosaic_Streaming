...
#     min_distance = recognition_t  # 최소 거리값을 임계값으로 초기화
#     broadcaster_name = 'unknown'
#     broadcaster_box = None
...
#         if cls == 5 and face:  # face 클래스가 선택된 경우
...
#                     broadcaster_box = (int(x1), int(y1), int(x2), int(y2))

#     # 방송인 얼굴에 대한 처리
#     # if broadcaster_name != 'unknown' and broadcaster_box:
#     #     x1, y1, x2, y2 = broadcaster_box
#     #     cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
#     #     cv2.putText(img, f'{broadcaster_name} {min_distance:.2f}', (x1, y1 - 10),
...
#         if cls == 5 and face:
#             if (int(x1), int(y1), int(x2), int(y2)) != broadcaster_box:
