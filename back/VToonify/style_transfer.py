        ...
        for det in results.xyxy[0]:
            ...
            if cls == 5 and face:  # 얼굴 클래스일 경우
                ...
                if name == 'unknown':
                    ...
                    broadcaster_candidates.append((name, distance, x1, y1, x2, y2))
            ...
            if cls == 4 and knife:  # knife 클래스가 선택된 경우
                img = apply_mosaic(img, (int(x1), int(y1)), (int(x2), int(y2)))
         # 방송자 처리 (가장 가까운 한 명만 방송자로 선택)
       # 방송자 처리 (가장 가까운 한 명만 방송자로 선택)
        if broadcaster_candidates:
            broadcaster_candidates.sort(key=lambda x: x[1])  # 거리 순으로 정렬
            chosen_broadcaster = broadcaster_candidates[0]  # 가장 가까운 사람 선택

            # 방송자로 선택된 사람
            # name, distance, x1, y1, x2, y2 = chosen_broadcaster
            # cv2.rectangle(img, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
            # cv2.putText(img, f'{name} {distance:.2f}', (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

            # 방송자가 아닌 나머지 사람들 모자이크 처리
            for other_candidate in broadcaster_candidates[1:]:
                other_name, other_distance, other_x1, other_y1, other_x2, other_y2 = other_candidate

                print(f"Applying VToonify to non-broadcaster at ({other_x1}, {other_y1})")
                landmarks = detect_landmarks_dlib(img, predictor, other_x1, other_y1, other_x2, other_y2)
                if landmarks is None:
                    continue
