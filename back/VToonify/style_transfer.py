...
def detect_and_stylize(img, model, face_encoder, encoding_dict,  device, style_degree, license_plate, invoice, id_card, license_card, knife, face):
    ...
    vtoonify.load_state_dict(torch.load('./VToonify/checkpoint/vtoonify_d_cartoon/vtoonify_s_d_c.pt', map_location=device)['g_ema'])
    vtoonify.to(device)

    pspencoder = load_psp_standalone('./VToonify/checkpoint/encoder.pt', device)
    exstyles = np.load('./VToonify/checkpoint/vtoonify_d_cartoon/exstyle_code.npy', allow_pickle=True).item()
    style_id = 26
